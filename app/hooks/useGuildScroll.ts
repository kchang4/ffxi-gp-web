...
interface UseGuildScrollReturn {
    ...
    scrollToGuild: (guildId: number, updateUrlCallback: (params: any) => void, closeSidebarCallback: () => void) => void;
    handleAutoScroll: (dataReady: boolean) => void;
}

export function useGuildScroll(): UseGuildScrollReturn {
    const [pendingScrollGuild, setPendingScrollGuild] = useState<number | null>(null);
    const [selectedGuild, setSelectedGuild] = useState<number | null>(null);
    const isProgrammaticScroll = useRef(false);

    const scrollToGuild = useCallback((guildId: number, updateUrlCallback: (params: any) => void, closeSidebarCallback: () => void) => {
